...
enum flags
{
  VARIABLE = (1 << 0),
  CONSTANT = ~(1 << 0),
  LEFT = (1 << 1),  /* Parenthesis, commands.  */
  RIGHT = ~(1 << 1), /* Parenthesis, comma, semicolon, etc.  */
  KEYWORD = (1 << 2), /* return, goto, etc.  */
  PREFIX = (1 << 3),
  POSTFIX = ~(1 << 3),
  UNARY = (1 << 4),
  BINARY = (1 << 5),
...
const op_t function_tab[] =
{
 ...
 set_op ("&", BINARY, bit_and),
 set_op ("|", BINARY, bit_or),
 set_op ("^", BINARY, bit_xor),
 set_op ("<<", BINARY, lsh),
 set_op (">>", BINARY, rsh),
...
bool skip_comment(const char **str)
{
  const char *s = *str;
  if (*s != '/' || *(s+1) != '*') {
        return false;
...
size_t skip_string(const char **str)
{
  const char *s = *str;
  size_t n = 0;
  if (*s == '\"')
    ...
    if (number_stack->n_used > 0 && (fr != NULL || fc != NULL))
      {
        ...
        if (i == 8 || i == 2) {
          ...
          if (i == 9 || i >= 400) {
            // Unary operator
            if (isMatrix(a1) || fc == NULL && fr == NULL) {
              if (fm == NULL) {
                cerror(err::Function_Do_Not_Support_Matrices);
              }
              else {
                (reinterpret_cast<TunaryC0>(fm))(a1);
              }
            }
            else {
              if (isImag(a1) || fr == NULL) {
...
{
  return is_latin_alpha (c) || c == '_' || signed(c) < 0 || c >= '0' && c <= '9';
}

//---------------------------------------------------------------------------

int find_label(const char *s, int len)
{
    label_t *a;
...
int cmp_search_operator (search_info_t *si, const op_t **po)
{
  const char *d, *a;
  char c;
  int i;
...
int cmp_search_operator_s (const char *s, const operator_t **po)
{
    search_info_t si;
    si.s = s;
    si.c1 = *s;
...
Token token(const char *s, const char **e, bool is_for, bool is_postfix)
{
  char c;
  const char *a;
  mpc_t x;
  const operator_t *o, **po, **po1;
  stack_t *t;
  variable_t *v;
  int i;
  ...
  if (!isLetter(*s) || isLetter(s[1])) {
    ...
    search_info.s = s;
    po = (const Top **) bsearch(&si, function_tab_sorted, countof (function_tab_sorted), sizeof(Top *), (int (*)(const void *, const void *))cmpSearchOp);
    if (po == NULL) {
      po = op_last_compared;
      if (isLetter(search_info.c1))
        {
          /* Compare prefixes.  */
          if (po > function_tab_sorted + 1 && !cmp_search_operator_s (*(po-1)->name, po - 2))
            po--;
          while (po > function_tab_sorted && !cmp_search_operator_s((*po)->name, po - 1))
            po--;
        }
      else
        {
          /* Compare all operators which start with the same character.  */
          while (po > function_tab_sorted && *(po-1)->name[0] == si.c1 && cmp_search_operator(&si, po))
            po--;
        }
...
init_all_stacks (void)
{
  ...
  stack_init (variable_stack, sizeof (variable_t), variable_stack_destroy);
  stack_init (label_stack, sizeof (label_t), NULL);
  stack_init (goto_stack, sizeof (goto_t), NULL);
  stack_init (function_stack, sizeof (function_t), function_stack_destroy);
}
